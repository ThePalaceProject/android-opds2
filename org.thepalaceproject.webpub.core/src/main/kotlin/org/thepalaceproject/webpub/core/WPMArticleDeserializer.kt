...
class WPMArticleDeserializer :
  StdDeserializer<WPMArticle>(WPMArticle::class.java) {
  ...
  override fun deserialize(
    ...
    return when (val currentToken = parser.currentToken()) {
      JsonToken.VALUE_STRING   -> {
        WPMArticle.WPMArticleString(parser.text)
      }
      JsonToken.START_ARRAY        -> {
        return this.consumeArray(parser, context)
      }
      JsonToken.START_OBJECT        -> {
        context.readValue(parser, WPMArticle.WPMArticleObject::class.java)
      }
  ...
  private fun consumeArray(
    parser : JsonParser,
    context : DeserializationContext
  ) : WPMArticle.WPMArticles {
    val results =
      mutableListOf<WPMArticle>()
      ...
      when (val currentToken = parser.nextToken()) {
        JsonToken.START_OBJECT -> {
          results.add(
            context.readValue(
              parser,
              WPMArticle.WPMArticleObject::class.java
            )
          )
        }

        JsonToken.VALUE_STRING -> {
          results.add(WPMArticle.WPMArticleString(parser.text))
        }
        ...
        JsonToken.END_ARRAY    -> {
          return WPMArticle.WPMArticles(results.toList())
        }

...
class WPMChapterDeserializer :
  StdDeserializer<WPMChapter>(WPMChapter::class.java) {
  ...
  override fun deserialize(
    ...
    return when (val currentToken = parser.currentToken()) {
      JsonToken.VALUE_NUMBER_INT   -> {
        WPMChapter.WPMChapterNumber(parser.bigIntegerValue)
      }

      JsonToken.VALUE_NUMBER_FLOAT -> {
        WPMChapter.WPMChapterNumber(parser.doubleValue)
      }
      ...
      JsonToken.START_OBJECT       -> {
        context.readValue(parser, WPMChapter.WPMChapterObject::class.java)
      }
  ...
  private fun consumeArray(
    parser : JsonParser,
    context : DeserializationContext
  ) : WPMChapter.WPMChapters {
    val results =
      mutableListOf<WPMChapter>()
      ...
      when (val currentToken = parser.nextToken()) {
        JsonToken.START_OBJECT       -> {
          results.add(
            context.readValue(
              parser,
              WPMChapter.WPMChapterObject::class.java
            )
          )
        }
        ...
        JsonToken.END_ARRAY          -> {
          return WPMChapter.WPMChapters(results.toList())
        }
